...
def _profile_data(user: User) -> dict:
    ...
    return {
        "activities": [{"name": item.activity.name, "skill_level": item.skill_level} for item in user.activities],
        "availability": [
            {"day": item.day_of_week, "start_time": item.start_time, "end_time": item.end_time}
...
def list_matches(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> list[Match]:
    ...
    if not matches:
        for candidate in db.query(User).filter(User.id != current_user.id, User.is_active.is_(True)).all():
            result = calculate_profile_match(_profile_data(current_user), _profile_data(candidate))
            if result.overall_score > 0:
                activity = next((a.activity for a in current_user.activities if a.activity.name.lower() in result.shared_activities), None)
                matches.append(Match(user_id=current_user.id, matched_user_id=candidate.id,
                                     activity_id=activity.id if activity else None,
                                     score=result.overall_score, explanation=" ".join(result.reasons)))
        db.add_all(matches)
        db.commit()
