...
from app.schemas.connection import (
    ConnectionCreate, ConnectionRead, ConnectionStatusUpdate, MessageCreate, MessageRead,
)
from app.services.connection_service import add_message, create_connection, update_connection
...
def request_connection(
    payload: ConnectionCreate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)
) -> Connection:
    if payload.recipient_id == current_user.id:
        raise HTTPException(status_code=400, detail="You cannot connect with yourself")
    if db.query(User).filter(User.id == payload.recipient_id, User.is_active.is_(True)).first() is None:
    ...
    if existing:
        raise HTTPException(status_code=409, detail="A connection already exists between these users")
    return create_connection(db, current_user.id, payload.recipient_id)


@router.get("", response_model=list[ConnectionRead])
def list_connections(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> list[Connection]:
...
def respond_to_connection(
    connection_id: int, payload: ConnectionStatusUpdate, current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
) -> Connection:
    connection = _get_member_connection(db, connection_id, current_user.id)
    if connection.recipient_id != current_user.id:
        raise HTTPException(status_code=403, detail="Only the recipient can respond to a connection request")
    if connection.status != "pending":
        raise HTTPException(status_code=409, detail="Connection request has already been handled")
...
@router.post("/{connection_id}/messages", response_model=MessageRead, status_code=status.HTTP_201_CREATED)
def send_message(
    connection_id: int, payload: MessageCreate, current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
) -> MessageRead:
    connection = _get_member_connection(db, connection_id, current_user.id)
    if connection.status != "accepted":
        raise HTTPException(status_code=403, detail="Messaging requires an accepted connection")
...
def list_messages(
    ...
    if connection.status != "accepted":
        raise HTTPException(status_code=403, detail="Messaging requires an accepted connection")
    return db.query(Message).filter(Message.connection_id == connection.id).order_by(Message.created_at.asc()).all()
