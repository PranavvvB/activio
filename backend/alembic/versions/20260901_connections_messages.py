...
def upgrade() -> None:
    op.create_table(
        ...
        sa.CheckConstraint("requester_id != recipient_id", name="ck_connection_not_self"),
    )
    for column in ("id", "requester_id", "recipient_id", "status"):
        op.create_index(op.f("ix_connections_{0}".format(column)), "connections", [column], unique=False)
    op.create_table(
        "messages",
        sa.Column("id", sa.Integer(), nullable=False),
    ...
    for column in ("id", "connection_id", "sender_id"):
        op.create_index(op.f("ix_messages_{0}".format(column)), "messages", [column], unique=False)
...
def downgrade() -> None:
    ...
    for column in ("status", "recipient_id", "requester_id", "id"):
        op.drop_index(op.f("ix_connections_{0}".format(column)), table_name="connections")
