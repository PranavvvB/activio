...
def make_user(db, email: str, username: str) -> User:
    user = User(email=email, username=username, password_hash=get_password_hash("password123"))
    db.add(user)
    db.commit()
    db.refresh(user)
    return user
...
def test_connection_request_acceptance_and_messaging(client, db_session):
    ...
    request = client.post("/api/connections", headers=headers(alice), json={"recipient_id": bob.id})
    ...
    blocked = client.post(
        ...
        headers=headers(alice), json={"content": "Hello"},
    )
    assert blocked.status_code == 403

    accepted = client.post(f"/api/connections/{connection['id']}/accept", headers=headers(bob))
    ...
    listed = client.get(f"/api/connections/{connection['id']}/messages", headers=headers(bob))
...
def test_connection_rejection_and_authorization(client, db_session):
    ...
    rejected = client.post(f"/api/connections/{connection['id']}/reject", headers=headers(bob))
...
def test_connection_validation_and_listing(client, db_session):
    ...
    assert client.post("/api/connections", headers=headers(alice), json={"recipient_id": alice.id}).status_code == 400
    assert client.post("/api/connections", headers=headers(alice), json={"recipient_id": 99999}).status_code == 404
